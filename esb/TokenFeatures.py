...
class TokenFeatures:
    ...
    def __emit_word_features(rel_pos, word, statement_label):
        features = {}
        # This captures the prev. predicted statement label
    ...
    def get_word_features(sentence, i):
        ...
        for x in range(i - 3, i + 4):
            if 0 <= x < len(sentence):
                rel_pos_str = str(-(i - x))
                features.update(
                    TokenFeatures.__emit_word_features(-(i - x), sentence[x][0], sentence[x][1]))
                features.update(
                    {rel_pos_str + ':same-statement-label': i_statement_label == sentence[x][1]})
    ...
    def get_sentence_features(sentence):
        return [
            TokenFeatures.get_word_features(
                sentence,
                i) for i in range(
                len(sentence))]
