...
class GraphWriter:
    def __init__(self):
        self._driver = GraphDatabase.driver("***REMOVED***", auth=("***REMOVED***", "***REMOVED***"))
    ...
    def to_cypher(entity):
        ...
        lines.append("MERGE (xx1:Account {account_number:\"" + account_id + "\"})")
        ...
        for k1, v1 in entity['extracted'].items():
            if k1 == "native_of_objects":
                for x in v1:
                    click = click + 1
                    key = "zza" + str(click)
                    lines.append("MERGE (" + key + ":Location {name: \"" + x['name'] + "\"})")
                ...
                for parent in v1:
                    ...
                    if isinstance(parent, dict):
                        ...
                        for k2, v2 in parent.items():
                            if k2 == "location":
                                lines.append("MERGE (z" + key + ":Location {name: \"" + v2['name'] + "\"})")
                                lines.append("CREATE (" + key + ")-[:LOCATED_AT]->(z" + key + ")")
                            elif k2 == "type":
                                if v2 == "Father":
                ...
                for journey in v1:
                    if 'vessel' in journey:
                        ...
                        if 'vessel' in vess:
                            click = click + 1
                            key = "ddo" + str(click)
                            lines.append("MERGE (" + key + ":Vessel {name: \"" + vess['vessel'] + "\"})")
                            lines.append("CREATE (a)-[:EMIGRATED_VIA]->(" + key + ")")
                            for origin in entity['extracted']['emig_origin']:
                                click = click + 1
                                loc_key = "aa" + key
                                lines.append("MERGE (" + loc_key + ":Location {name: \"" + origin['name'] + "\"})")
                                lines.append("CREATE (" + key + ")-[:VESSEL_DEPARTED_FROM]->(" + loc_key + ")")
        return "\n".join(lines)
        ...
        for k1, v1 in entity['extracted'].items():
            if k1 == "native_of":
                for k2, v2 in v1.items():
                    if k2 == "location":
                        lines.append("MERGE (zz1:Location {name: \"" + v2 + "\"})")
                ...
                for parent in v1:
                    ...
                    if isinstance(parent, dict):
                        ...
                        for k2, v2 in parent.items():
                            if k2 == "location":
                                lines.append("MERGE (z" + key + ":Location {name: \"" + v2 + "\"})")
                                lines.append("CREATE (" + key + ")-[:LOCATED_AT]->(z" + key + ")")
                            elif k2 == "type":
                                if v2 == "Father":
                                    lines.append("CREATE (a)-[:HAS_FATHER]->(" + key + ")")
